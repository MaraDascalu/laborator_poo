...
Magazin::Magazin(std::string nume,std::vector<std::unique_ptr<produs_cantitate>> &&produse, int  capacitate) : nume(nume),  capacitate_maxima(capacitate){
    for (auto &&produs: produse)
        this -> produse.emplace_back(std::move(produs));
}
...
std::vector<std::unique_ptr<produs_cantitate>>& Magazin::get_produse(){
    return produse;
}
